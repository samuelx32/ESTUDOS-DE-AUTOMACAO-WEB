...
async def auto1 (wb):
    ...
    async with async_playwright() as p:        
        browser = await p.chromium.launch(args=["--window-position=100,100"],channel="chrome", headless=False, downloads_path=Diretorio)    
        
        #time.sleep(4)
        CEP = await browser.new_page()
        CEP.set_default_timeout(5000)

        await CEP.set_viewport_size({'width': 700, 'height': 800})
        ...
        for i in range(0,qtn):
            titulo = await CEP.locator(f"#projects > .project-item > div > h2 > a > span >> nth = {i}").inner_text()
...
async def auto2 (wb):
    ...
    from playwright.async_api import async_playwright, expect
    async with async_playwright() as p:        
        browser = await p.chromium.launch(args=["--window-position=900,100"],channel="chrome", headless=False, downloads_path=Diretorio)    
