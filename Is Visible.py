...
def run(playwright: Playwright) -> None:
    ...
    for linha in range(1,ultima_linha):                       
        ...
        # Verificando se elemento existe na tela (is_visible())
        if page.get_by_text("Processamento realizado totalmente!").is_visible() == True:
            planilha['Processamento'][linha].value="Processamento Total"
        elif page.get_by_text("Processamento realizado parcialmente!").is_visible() == True:
            planilha['Processamento'][linha].value="Processamento Parcial"
...
def lidando_com_elementos_especificos():
    with sync_playwright() as playwright:
        ...
        expect(page.get_by_role("button", name="Enviar Relatório")).to_be_visible(timeout=4000)
        page.get_by_role("button", name="Clicar após processamento").click()
